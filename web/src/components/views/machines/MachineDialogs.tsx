  ...
  return (
    <fieldset className="border border-line rounded p-0 m-0">
      ...
      <div className="grid gap-[0.45rem] p-[0.7rem]">
        ...
        {sshKeys.map((key) => <label key={key.name} className="flex items-center gap-[0.45rem] text-[0.84rem] cursor-pointer"><input type="checkbox" className="w-[0.95rem] h-[0.95rem] m-0 accent-brand" checked={preset.sshKeyRefs.includes(key.name)} onChange={() => onToggleSSHKeyRef(key.name)} />{key.name}</label>)}
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-[0.45rem]">
