...
function Toolbar({
  ...
  return (
    <div className="flex items-center gap-2 pt-3 px-[14px] pb-2">
      ...
      />
      <button
        className="shrink-0 bg-brand border-brand-strong text-white py-[6px] px-[10px] text-[11.5px] font-medium"
        disabled={quickDeploying}
        onClick={onQuickDeploy}
      >
        {quickDeploying ? 'Deploying…' : 'Deploy'}
      </button>
      <button className="shrink-0 py-[6px] px-2 text-[11.5px]" disabled={quickDeploying} onClick={onOpenQuickDeploySettings}>
        Preset
      </button>
